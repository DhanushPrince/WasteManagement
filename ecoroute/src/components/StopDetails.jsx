...
// S3 path e.g. s3://internal-testing-1/images/2.jpeg -> /api/s3-image?key=images/2.jpeg
// Else local: /dump/filename
function resolveImageSrc(imagePath) {
  if (!imagePath) return null;
  const path = imagePath.replaceAll("\\", "/").trim();
...
StopDetails.propTypes = {
  stop: PropTypes.shape({
    ...
    volume_level: PropTypes.string,
    area_covered_sqm: PropTypes.number,
    ...
    nearest_zone: PropTypes.shape({
      ...
      distance_m: PropTypes.number,
    }),
    high_value_recyclables_present: PropTypes.bool,
    recyclable_types: PropTypes.arrayOf(PropTypes.string),
    reasoning: PropTypes.string,
    confidence_score: PropTypes.number,
    image_quality: PropTypes.string,
    gps_accuracy_m: PropTypes.number,
    requires_after_photo: PropTypes.bool,
